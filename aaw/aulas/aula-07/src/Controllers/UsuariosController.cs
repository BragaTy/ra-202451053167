...
public class UsuariosController : ControllerBase
{
    ...
    [Authorize]
    [HttpPut("{id}")]
    public IActionResult Atualizar(int id, [FromBody] AtualizarUsuarioDto dados)
    {
        var usuario = _db.Usuarios.Find(id);
        if (usuario is null) return NotFound();

        // Só o próprio usuário ou um Admin podem atualizar este recurso
        var usuarioIdToken = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
        var roleToken = User.FindFirst(ClaimTypes.Role)?.Value;

        if (usuarioIdToken != id.ToString() && roleToken != "Admin")
            return Forbid();
