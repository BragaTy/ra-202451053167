...
public IActionResult GetById(int id)
{
    ...
    // Pegar o ID do usuário autenticado do token
    var usuarioIdToken = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
    
    // Validar se o pedido pertence ao usuário do token
    if (usuarioIdToken != pedido.UsuarioId.ToString())
    ...
    public IActionResult GetByUsuario(int usuarioId)
    {
        ...
        if (usuarioIdToken != usuarioId.ToString())
            return Forbid();
